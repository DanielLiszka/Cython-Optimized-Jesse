...
#from jesse.services.selectors import selectors
...
class CandlesState: 
    ...
    def add_one_candle(
        ...
        #initial 
        # print(f'candle timestamp {candle[0]}')
        if old_index == -1:
            arr.append(candle)
        ...
        elif time_candle == arr.array[arr.index-1][0]:
            print(f'error! {time_candle} == {arr.array[arr.index-1][0]}')
            # loop through the last 20 items in arr to find it. If so, update it.
            for i in range(max(20, len(arr.array) - 1)):
                if arr.array[arr.index-i][0] == candle[0]:
                    arr.array[arr.index-i] = candle
                    break
                else:
                    logger.info(
                        f"Could not find the candle with timestamp {jh.timestamp_to_time(candle[0])} in the storage. Last candle's timestamp: {jh.timestamp_to_time(arr.array[-1][0])}. timeframe: {timeframe}, exchange: {exchange}, symbol: {symbol}"
                    )
    ...
    def add_multiple_candles(self,
        ...
        # allow updating of the previous candle
        elif candle[-1,0] == arr.array[arr.index-1][0]:
            print(f'error! {candle[-1,0]} == {arr.array[arr.index-1][0]}')
            # loop through the last 20 items in arr to find it. If so, update it.
            for i in range(max(20, len(arr.array) - 1)):
                if arr.array[arr.index-i][0] == candle[0]:
                    arr.array[arr.index-i] = candle
    ...
    def batch_add_candle(
        ...
        # pd.DataFrame(candles).to_csv('batch_candles.csv')
        for c in candles:
    ...
    # # # # # getters
    # # # # # # # # #
    def get_candles(self, exchange: str, symbol: str, timeframe: str, bint fullonly=False) -> np.ndarray:
        ...
        if dif == 0 or self.storage[long_key].getslice(0,long_count)[-1][0] == self.storage[short_key].array[short_count-dif][0]:
            if fullonly and dif != 0:
                ...
                return self.storage[long_key].getslice(0,long_count-1)
            else:
    ...
    def get_current_candle(self, exchange: str, symbol: str, timeframe: str, bint fullonly=False) -> np.ndarray:
        cdef int long_count, short_count, dif
        # no need to worry for forming candles when timeframe == 1m
        if timeframe == '1m':
            arr: c_DynamicNumpyArray = self.storage[f'{exchange}-{symbol}-{"1m"}']
        ...
        if dif != 0:
            if fullonly:
                ...
                if long_count <= 1:
                    ...
                    return self.storage[long_key].array[self.storage[long_key].index-1]
            else:        
...
class DynamicNumpyArray:
    ...
    def append_multiple(self, np.ndarray[DTYPE_t, ndim=2] items) -> None:
        self.index += 1
