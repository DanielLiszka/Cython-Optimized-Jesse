...
{{!orderData}}
    ...
    const getCustomData = async (offset) => {
        ...
        {{!customCharts}}
          ...
          function filterLineSeriesValues(lineSeriesValues, seriesObjects, bar) {

              let filteredValues = lineSeriesValues.filter((item) => {
                  const difference = Math.abs(item.value - bar.close);
                  const percentDifference = (difference / bar.close) * 100;
                  return percentDifference <= 50;  // Keep values where the difference is 50% or less
              });

              // Renumber the names to be consecutive
              for (let i = 0; i < filteredValues.length; i++) {
                  filteredValues[i].name = `indicator${i + 1}`;
              }
...
for (let data of lineSeriesValues) {
    ...
    if (validCrosshairPoint && bar.time !== undefined) {
        ...
        for (let entry of new_array) {
            let value = entry[1].value.toFixed(2);
            let seriesName = `indicator${++lineSeriesCount}`;
            lineSeriesValues.push({name: seriesName, value: value});
        }

        lineSeriesValues = filterLineSeriesValues(lineSeriesValues, seriesObjects, bar);
