...
<html lang="en">
  ...
  <body id="main-body" style="background-color: white">
    ...
    <div class="utility-box" id="utilityBox">
      ...
      <div id="toggleFibonacci">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
          <line x1="2" y1="2" x2="22" y2="2" stroke="white" stroke-width="2"/>
          <line x1="2" y1="6" x2="22" y2="6" stroke="white" stroke-width="2"/>
          <line x1="2" y1="10" x2="22" y2="10" stroke="white" stroke-width="2"/>
          <line x1="2" y1="14" x2="22" y2="14" stroke="white" stroke-width="2"/>
          <line x1="2" y1="18" x2="22" y2="18" stroke="white" stroke-width="2"/>
          <line x1="2" y1="22" x2="22" y2="22" stroke="white" stroke-width="2"/>
        </svg>
      </div>
    ...
    <div id="metricsOverlay" style="display: none">
      ...
      >
        <span style="flex-grow: 1; text-align: center; font-size: larger"
          ><strong>Performance Metrics</strong></span
        >
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const populateMetricsOverlay = () => {
              ...
              function drawFibonacci() {
                  // Remove existing Fibonacci lines if any
                  if (fibonacciLines.length > 0) {
                      fibonacciLines.forEach(line => candleseries.removePriceLine(line));
                      fibonacciLines = [];
                  }

                  if (!fibonacciVisible) return;

                  // Get the visible range of the chart
                  const visibleRange = chart.timeScale().getVisibleRange();

                  // Filter the candle data to get the data within the visible range
                  const visibleCandleData = klinedata.filter(candle => 
                      candle.time >= visibleRange.from && candle.time <= visibleRange.to
                  );

                  // Calculate the high and low within the visible range
                  let high = -Infinity;
                  let low = Infinity;
                  visibleCandleData.forEach(candle => {
                      if (candle.high > high) high = candle.high;
                      if (candle.low < low) low = candle.low;
                  });

                  // Calculate Fibonacci levels and titles
                  const fibonacciLevels = [
                      { level: 0, title: '0%' },
                      { level: 0.236, title: '23.6%' },
                      { level: 0.382, title: '38.2%' },
                      { level: 0.5, title: '50%' },
                      { level: 0.618, title: '61.8%' },
                      { level: 0.786, title: '78.6%' },
                      { level: 1, title: '100%' }
                  ];

                  fibonacciLevels.forEach(({ level, title }) => {
                      const priceLevel = low + (high - low) * level;
                      const fibonacciLine = candleseries.createPriceLine({
                          price: priceLevel,
                      ...
                      fibonacciLines.push(fibonacciLine);
                  });
              }

              // Subscribe to visible time range change event to redraw Fibonacci lines dynamically
              chart.timeScale().subscribeVisibleTimeRangeChange(drawFibonacci);

              // Function to toggle Fibonacci levels
              function toggleFibonacci() {
                  fibonacciVisible = !fibonacciVisible;
                  drawFibonacci();  // Redraw Fibonacci lines based on the new visibility state
              }

              document.getElementById('toggleFibonacci').addEventListener('click', toggleFibonacci);
            ...
            function toggleFibonacci() {
                ...
                fibonacciLines.forEach(line => {
                    line.applyOptions({ visible: fibonacciVisible });
                });
            }
