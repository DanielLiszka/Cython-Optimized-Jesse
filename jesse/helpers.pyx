...
def get_candle_source(candles: np.ndarray, source_type: str = "close") -> np.ndarray:
    ...
    if source_type == "close":
        ...
        return (candles[:,2] + candles[:,2] + candles[:,3] + candles[:,4])/4
...
def max_timeframe(timeframes_list) -> str:
    ...
    if timeframes.MINUTE_10 in timeframes_list:
        return timeframes.MINUTE_10
    ...
    if timeframes.MINUTE_2 in timeframes_list:
        return timeframes.MINUTE_2 
